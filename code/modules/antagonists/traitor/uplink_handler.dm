...
/datum/uplink_handler/proc/can_purchase_item(mob/user, datum/uplink_item/to_purchase)
	...
	var/is_martyr = owner?.has_objective(/datum/objective/martyr)
	if(telecrystals < to_purchase.cost || stock <= 0 || !is_martyr ? world.time < to_purchase.progression_minimum : FALSE)
		return FALSE
